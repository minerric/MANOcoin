...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "ManoNetwork") type = MANONetwork;
    if(strType == "ManoNetworkVariable") type = MANONetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == MANONetwork) s = "ManoNetwork";
    if(type == MANONetworkVariable) s = "ManoNetworkVariable";
