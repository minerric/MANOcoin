...
#if defined(NDEBUG)
# error "MANO Core cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "MANO Signed Message:\n";
  ...
  int halvings = nHeight / consensusParams.nSubsidyHalvingInterval;
    ...
    if (halvings >= 30)
        return 0;
          ...
          if (nHeight == 1)
              return 378432 * COIN;  // MANO PREMINE 3 % of 12614400
          if (nHeight >= 2 && nHeight <= 15)
              return 1 * COIN;  // Instamine Protection 1 MANO per block
          if (nHeight >= 16 && nHeight <= 30)
              return 2 * COIN;  // Instamine Protection 2 MANO per block
          if (nHeight >= 31 && nHeight <= 90)
              return 4 * COIN; // Instamine Protection 4 MANO per block
          if (nHeight >= 91 && nHeight <= 150)
              return 6 * COIN; // Instamine Protection 6 MANO per block
          if (nHeight >= 151 && nHeight <= 210)
              return 8 * COIN; // Instamine Protection 8 MANO per block


    // LogPrintf("height %u diff %4.2f reward %d\n", nPrevHeight, dDiff, nSubsidyBase);
      CAmount nSubsidy = 10 * COIN;

      // Subsidy is cut in half every 255500 blocks which will occur approximately every year.
    nSubsidy >>= halvings;
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;
                                                                      ...
                                                                      // mainnet:
    if(nHeight > nMNPIBlock)               ret += blockValue / 20; //  starts at (5 + 25) 30% on block 95.
    if(nHeight > nMNPIBlock+(nMNPIPeriod)) ret += blockValue / 5; // increases to (30 + 20) 50% after 30 blocks (125) - Payment enforcement activation is 1528156800
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // MANO : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(MANO): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(MANO): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // MANO : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(MANO): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(MANO): spork is off, skipping transaction locking checks\n");
    }
