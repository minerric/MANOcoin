...
Format: ```mngovernance prepare proposal-name url payment-count block-start mano-address monthly-payment-mano```
...
Format: ```mngovernance submit proposal-name url payment-count block-start mano-address monthly-payment-mano fee-tx```
...
When block ```1000000``` is reached you'll receive a payment for ```1200``` MANO to ```C6R9oN12KnB9zydzTLc3LikD9cCjjQzYG7```.
...
The following RPC commands are supported:
 ...
 - mngovernance "command"... ( "passphrase" )
  ...
  - vote               - Vote on a proposal by single masternode (using mano.conf setup)
